...
parser = argparse.ArgumentParser(description="DualNet")
...
def main():
    ...
    # Build model
    net = DualNet(channels=3, num_of_layers=opt.num_of_layers)
    criterion = sum_squared_error()
    # Move to GPU
    device_ids = [0]
    model = nn.DataParallel(net, device_ids=device_ids).to(device)
    optimizer = optim.Adam(model.parameters(), lr=opt.lr, eps=1e-8, betas=[0.9,0.999])          # original
    # training
    # step = 0                                                  #      ******************** comment
    ...
    for epoch in range(opt.epochs):
        ...
        # train
        for i, data in enumerate(loader_train, 0):          # original
...
if __name__ == "__main__":
    if opt.preprocess:
        if opt.mode == 'S':
            prepare_data(data_path='data', patch_size=41, stride=40, aug_times=4) #tcw201810102244
        if opt.mode == 'B':
